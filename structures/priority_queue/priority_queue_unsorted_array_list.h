...
namespace structures
{
	...
	inline T PriorityQueueUnsortedArrayList<T>::pop()
	{
		...
		int lastIndex = PriorityQueueList<T>::list_->size() - 1;

		if (index != lastIndex)
		{
			DSRoutines::swap((*PriorityQueueList<T>::list_)[index], (*PriorityQueueList<T>::list_)[lastIndex]);
		
		}

		PriorityQueueItem<T>* item = PriorityQueueList<T>::list_->removeAt(lastIndex);
