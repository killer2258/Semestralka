...
namespace structures
{
    ...
    {
        LinkedListItem<T>* newNode = new LinkedListItem<T>(data);
    ...
    inline void LinkedList<T>::insert(const T& data, const int index)
    {
        ...
        {
            LinkedListItem<T>* beforeNew = getItemAtIndex(index - 1);
            newItem->setNext(beforeNew->getNext());
            beforeNew->setNext(newItem);
        }
    ...
    {
        int indexOfData = getIndexOf(data);
        if (indexOfData == -1)
            return false;

        removeAt(indexOfData);
        return true;
    }
